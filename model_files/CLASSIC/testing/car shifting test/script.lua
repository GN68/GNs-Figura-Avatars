...
gearPitchProfile = {1,3,7,12,16,19}

throttle = keybind.newKey("throttle","U")
gearUp = keybind.newKey("gear shift up", "I")
gearDown = keybind.newKey("gear shift up", "Y")

function tick()
    sinceLastSound = sinceLastSound - 1
    pitch = (engine / gearPitchProfile[gear])*0.005
    if sinceLastSound < 0 then
        sound.playCustomSound("Engine",player.getPos(),{pitch+0.3,0.5})
        sinceLastSound = 0.9/(pitch+1)
        if engine > gearPitchProfile[gear]*200 then
            engine = (gearPitchProfile[gear]*200)-10
        end
    end
    ...
    if gearUp.wasPressed() then
        gear = math.min(gear + 1,6)
    end
    if gearDown.wasPressed() then
