--[Clothing alias index, in other words variables to shortcut switches between clothes and armours]
SUIT_CLOTHING = model.ClothesSuit2
SUIT_CLOTHING_DIGI = model.ClothesSuitDigi
SUIT_CLOTHING_MERMAID= model.ClothesSuitMermaid
UNDER_ARMOR = model.UnderArmor
...
function ping.underarmorSwitch(val)
    
	underarmored = val
    UNDER_ARMOR.setEnabled(underarmored)
...
--ping.suittogglemain(false)

function ping.suitSwitch(suitval)
    itemsuit = item_stack.createItem("minecraft:coal_ore")
    enablesuit = not enablesuit
    if not enablesuit then
        wordsuit = "Disabled"
        itemsuit = item_stack.createItem("minecraft:stone")
        --if enable_tail == false then
        SUIT_CLOTHING.setEnabled(false)
        SUIT_CLOTHING_DIGI.setEnabled(false)
        SUIT_CLOTHING_MERMAID.setEnabled(false)
        --end
        ping.underarmorSwitch(false)
        action_wheel.SLOT_1.setTitle("Suit "..wordsuit, itemsuit)
        action_wheel.SLOT_1.setItem(itemsuit)
    else
        wordsuit = "Enabled"
        itemsuit = item_stack.createItem("minecraft:coal_ore")
        SUIT_CLOTHING_MERMAID.setEnabled(false)
        SUIT_CLOTHING_DIGI.setEnabled(true)
        SUIT_CLOTHING.setEnabled(true)
        --end
        ping.underarmorSwitch(false)
        action_wheel.SLOT_1.setTitle("Suit "..wordsuit, itemsuit)
        action_wheel.SLOT_1.setItem(itemsuit)
...
function ping.undercalvesSwitch(valo)
	undercalves = valo
...
chest = {model.base.RightArm.armor, model.base.Body.bodyarmor, model.base.BodyDigitigrade.bodyarmorDigi},
...
legs = {model.base.LeftLeg.leftlegarmor.leggings, model.base.Body.Leggings.Armor, model.base.Body.Leggings.ArmorTrim,
    ...
    model.base.Body.Leggings.bone.armor1, model.base.BodyDigitigrade.tailonland2.tailarmor,
...
if enablesuit then
    SUIT_CLOTHING_MERMAID.setEnabled(enable_tail)
    SUIT_CLOTHING_DIGI.setEnabled(not(enable_tail))
end
end
...
suitcustomis = action_wheel_pages.createFolder("Suit Customization", "minecraft:gilded_blackstone")
...
ping.toggleSitDown = function (toggle)
    ...
    if is_sitting_down then
        animation.sit.play()
        camera.FIRST_PERSON.setPos({0,-0.5,0})
        camera.THIRD_PERSON.setPos({0,-0.5,0})
    else
        animation.sit.stop()
        animation.sitL.stop()
        animation.sitR.stop()
...
greencalves.setFunction(function () ping.Calves("green") end)
whitecalves = action_wheel_pages.createItem("White", "minecraft:white_wool")
whitecalves.setFunction(function () ping.Calves("white") end)
blackcalves = action_wheel_pages.createItem("Black", "minecraft:black_wool")
blackcalves.setFunction(function () ping.Calves("black") end)
redcalves = action_wheel_pages.createItem("Red", "minecraft:red_wool")
redcalves.setFunction(function () ping.Calves("red") end)
bluecalves = action_wheel_pages.createItem("Blue", "minecraft:blue_wool")
bluecalves.setFunction(function () ping.Calves("blue") end)
...
--------------------------------------
--                                  --
-- ACTION WHEEL FOLDER CLOTHING :   --
--                                  --
--     CLOTHING ALIAS INDEX         --
--                                  --
--------------------------------------
...
--------------------------------------
--                                  --
-- ACTION WHEEL FOLDER CLOTHING : SUIT
--                                  --
--------------------------------------
...
enablesuit = true
--SUIT_CLOTHING_MERMAID.setEnabled(enable_tail)
--SUIT_CLOTHING.setEnabled(not(enable_tail))
wordsuit = "Disabled"
--suit = {model.base.BodyDigitigrade.clothessuit}
itemsuit = item_stack.createItem("minecraft:stone")
suittogglemain = action_wheel_pages.createItem("Suit "..wordsuit, itemsuit)
suittogglemain.setFunction(ping.suitSwitch)
ping.underarmorSwitch(false)
apparel.add(suitcustomis)
suitcustomis.add(suittogglemain)
