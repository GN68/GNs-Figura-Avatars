...
throt = keybind.newKey("Throttle","U")
...
function simulation()
    if startup then
        ...
        for key, p in pairs(point) do
            ...
            p.pos = p.pos + vectors.of({0,-0.08,0})
            p.pos = p.pos+(vectors.of({-0.1,0.1,0})*throttle)
...
function tick()
    log(throttle)
    if not throt.isPressed() and not reverse.isPressed() then
        throttle = lerp(throttle,0,0.1)
    end

    if throt.isPressed() then
        throttle = lerp(throttle,1,0.1)
    end
    ...
    if not steerLeft.isPressed() and not steerRight.isPressed() then
        throttle = lerp(throttle,0,0.1)
    end

    if steerLeft.isPressed() then
        steer = lerp(throttle,45,0.1)
    end
