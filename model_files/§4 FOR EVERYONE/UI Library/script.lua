...
function player_init()
    GNUI.set_anchor(model.HUD.button1,{-1,-1},{0,0})
    local example = GNUI.UI.button.new("example1",model.HUD.button1)
    example.area = {0,0,38,48,-1,-1}
    example.label.text = "text"
    example.groupRoot = model.HUD

    GNUI.set_anchor(model.HUD.button2,{0,0},{0,0})
    local pangalawa = GNUI.UI.button.new("pangalawa",model.HUD.button2)
    pangalawa.area = {-38,-28,76,48,0,0}
    pangalawa.groupRoot = model.HUD

    GNUI.set_anchor(model.HUD2.button3,{1,-1},{0,0})
    local toggleButton = GNUI.UI.button.new("toggleButton",model.HUD2.button3)
    toggleButton.area = {-40,0,40,47,1,-1}
    toggleButton.pressed_function = function ()
        model.HUD.setEnabled(not model.HUD.getEnabled())
    end
end
