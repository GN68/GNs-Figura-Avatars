...
function player_init()
    GNUI.set_anchor(model.HUD.button1,{-1,1},{0,-12})
    local gameModePanel = GNUI.UI.button.new("gameModePanel",model.HUD.button1)
    gameModePanel.area = {0,0,16,64,-1,1}
end
...
function GNUI.buttonTick(elmnt)
    ...
    if GNUI.Config.showButtonAreas and type(elmnt.area) ~= "nil" then
        if windowSizeChanged then
            ...
            if not model.HUD_DEBUG.getRenderTask(elmnt.dynamicValues.id) then
               
                
                model.HUD_DEBUG.addRenderTask("BLOCK",elmnt.dynamicValues.id,"minecraft:red_stained_glass",true,{
                    result.x,
                    result.y,zDepth
                },{},
                {elmnt.area[3]/40,elmnt.area[4]/40,1})
            else
                model.HUD_DEBUG.getRenderTask(elmnt.dynamicValues.id).setPos{
                    result.x,
                    result.y,zDepth
                }
            end
